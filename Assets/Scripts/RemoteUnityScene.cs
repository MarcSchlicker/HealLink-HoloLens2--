...
public class RemoteUnityScene : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        ConfigureVisibleMaterial(m_material, Color.white);
    }
    ...
    uint MSG_CreatePrimitive(byte[] data)
    {
        ...
        renderer.material = m_material != null ? new Material(m_material) : new Material(Shader.Find("Standard"));
        ConfigureVisibleMaterial(renderer.material, Color.white);
        go.SetActive(false);
    ...
    uint MSG_SetColor(byte[] data)
    {
        ...
        Color color = new Color(BitConverter.ToSingle(data, 4), BitConverter.ToSingle(data, 8), BitConverter.ToSingle(data, 12), BitConverter.ToSingle(data, 16));
        Renderer renderer = go.GetComponent<Renderer>();
        if (renderer != null)
        {
            ConfigureVisibleMaterial(renderer.material, color);
        }

        return 1;
    }

    static void ConfigureVisibleMaterial(Material material, Color color)
    {
